...
export const useDailyRegistrationForm = ({ setView }: UseDailyRegistrationFormProps) => {
  ...
  const handleConfirmRegistration = useCallback(async (data: Omit<MealRegistration, 'id'>[]) => {
    try {
        ...
        if (isEditing || isOverwrite) {
            // For both edits and overwrites, we use the smart update function.
            // It needs the original state to compare against.
            ...
            // Only for brand new registrations (no existing data to overwrite)
        ...
        if (error.message === 'STALE_DATA') {
            // Toast is already shown by context, just reset form
            resetForm();
        }
        // Other errors are handled and toasted in the context
    }
  ...
  const handleSubmit = useCallback(async (e: FormEvent) => {
    ...
    // Overwrite check for non-editing mode
