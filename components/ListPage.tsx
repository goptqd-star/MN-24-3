...
const ListSummaryStatsBox: React.FC<{ totals: Record<MealType, number> | null }> = ({ totals }) => {
    const totalAll = totals ? Object.values(totals).reduce((sum, count) => sum + count, 0) : 0;
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-center">
            <div className="bg-teal-50 dark:bg-teal-900/50 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-teal-600 dark:text-teal-400">{(totals?.[MealType.KidsLunch] || 0).toLocaleString('vi-VN')}</p>
            </div>
            <div className="bg-indigo-50 dark:bg-indigo-900/50 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{(totals?.[MealType.TeachersLunch] || 0).toLocaleString('vi-VN')}</p>
            </div>
            <div className="bg-pink-50 dark:bg-pink-900/50 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-pink-600 dark:text-pink-400">{(totals?.[MealType.KidsBreakfast] || 0).toLocaleString('vi-VN')}</p>
            </div>
            <div className="bg-gray-100 dark:bg-gray-700/50 p-4 rounded-lg border-2 border-dashed">
                ...
                <p className="text-2xl font-extrabold text-gray-900 dark:text-gray-100">{totalAll.toLocaleString('vi-VN')}</p>
            </div>
        </div>
    );
};
  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        registrations.forEach(r => {
            if (r.date === currentDayStr && r.count > 0) {
                registeredClassNames.add(r.className);
            }
        });
        
        let relevantClasses = classes.filter(c => registeredClassNames.has(c.name));
        if (currentUser?.role === Role.GV && currentUser.assignedClass) {
            relevantClasses = relevantClasses.filter(c => c.name === currentUser.assignedClass);
        }
        setFilterableClasses(relevantClasses);

        if (isInitialLoad && relevantClasses.length > 0) {
        ...
        const combinedData = Array.from(registeredClassNames).map(className => {
                 ...
                 <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {paginatedData.map(item => (
                            <ListTableRow key={item.className} data={item} isHighlighted={recentlyUpdatedKeys.has(item.className)} />
                        ))}
                    </tbody>
                </table>
            ) : (
