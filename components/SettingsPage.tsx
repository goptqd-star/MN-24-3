...
const SettingsPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (isModalOpen && modalRef.current) {
            ...
            // Replaced querySelectorAll<HTMLElement> with a type assertion to fix the error.
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead className="bg-gray-50 dark:bg-gray-700">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Số trẻ</th>
                                ...
                                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                    {filteredClasses.map(c => (
                                        <tr key={c.id}>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-teal-700 dark:text-teal-400">{c.name}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-600 dark:text-gray-300">{c.studentCount}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-4">
                                                <button onClick={() => setEditingClass(c)} className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200">Sửa</button>
                                                <button onClick={() => setDeletingClass(c)} className="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-200">Xóa</button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                            ...
                            {filteredClasses.length === 0 && (
                                <p className="text-center text-gray-500 dark:text-gray-400 py-6">
                                    {isLoading ? 'Đang tải...' : 'Không tìm thấy lớp học nào.'}
                                </p>
                            )}
                        </div>
                    </div>
