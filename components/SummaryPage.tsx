...
const SummaryPage: React.FC<{setView: (view: View) => void}> = ({setView}) => {
  ...
  const [paginatedRegistrations, setPaginatedRegistrations] = useState<MealRegistration[]>([]);
  const [reportTotals, setReportTotals] = useState<Record<MealType, number> | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  
  const [totalPages, setTotalPages] = useState(0);
  const [totalRecords, setTotalRecords] = useState(0);
  const [pageCursors, setPageCursors] = useState<Record<number, DocumentSnapshot<DocumentData> | null>>({});
  ...
  useEffect(() => {
    const fetchPageData = async () => {
        ...
        const { registrations, lastDoc, totalCount } = await getRegistrations({
            dateRange: (dateRange.from && dateRange.to) ? dateRange : undefined,
            classNames: selectedClasses.length > 0 ? selectedClasses : undefined,
            limit: ITEMS_PER_PAGE,
            lastVisibleDoc: pageCursors[currentPage - 1] || null
        });
        
        setPaginatedRegistrations(registrations);
        setTotalRecords(totalCount);
        setTotalPages(Math.ceil(totalCount / ITEMS_PER_PAGE));

        if (lastDoc) {
             setPageCursors(prev => ({ ...prev, [currentPage]: lastDoc }));
        }
    ...
    if (classes.length > 0 && selectedClasses.length > 0) {
      ...
      setPaginatedRegistrations([]);
      setTotalRecords(0);
      setTotalPages(0);
      setReportTotals(null);
      setIsLoading(false);
    }
  }, [currentPage, dateRange, selectedClasses, classes, getRegistrations, dataVersion]);
  ...
  }, [dateRange, selectedClasses]);

  const { paginatedData, isAllOnPageSelected } = useMemo(() => {
    const dataByDateAndClass = paginatedRegistrations.reduce((acc: Record<string, SummaryData>, reg: MealRegistration) => {
  ...
  }, [paginatedRegistrations, classes, selectedRows]);
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead className="bg-gray-50 dark:bg-gray-700 sticky top-0 z-10">
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-3 text-center align-top text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Lớp</th>
                      <th scope="col" className="px-6 py-3 text-center align-top text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Ngày</th>
                      <th scope="col" className="px-6 py-3 text-center align-top text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">{MealType.KidsBreakfast}</th>
                      <th scope="col" className="px-6 py-3 text-center align-top text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">{MealType.KidsLunch}</th>
                      <th scope="col" className="px-6 py-3 text-center align-top text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">{MealType.TeachersLunch}</th>
                 ...
                 <PaginationControls currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} className="no-print" />
                </>
             ) : (
