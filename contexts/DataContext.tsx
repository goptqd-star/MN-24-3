...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeAnnouncements = onSnapshot(announcementsQuery, (snapshot) => {
        ...
        const unreadCount = announcementData.filter(a => !a.readBy.includes(currentUser.id)).length;
  ...
  const exportData = useCallback(async (options: { format: 'csv' | 'pdf', dateRange: { from: string, to: string }, classNames: string[] }) => {
        ...
        try {
            ...
            const dataByDateAndClass = allRegistrations.reduce<Record<string, ExportableRow>>((acc, reg) => {
            ...
            const exportableData = Object.values(dataByDateAndClass).sort((a, b) => b.date.localeCompare(a.date) || a.className.localeCompare(b.className));
  ...
  const value = useMemo(() => ({
    editingInfo, classes, users, announcements, unreadAnnouncementsCount, dataVersion, recentlyUpdatedKeys, showBackupPrompt,
  ...
  }), [editingInfo, classes, users, announcements, unreadAnnouncementsCount, dataVersion, recentlyUpdatedKeys, showBackupPrompt, addRegistrations, updateRegistrations, requestEdit, clearEditing, deleteRegistrations, deleteMultipleRegistrationsByDate, addClass, updateClass, deleteClass, getRegistrations, addUser, updateUser, deleteUser, addAnnouncement, updateAnnouncement, deleteAnnouncement, markAnnouncementsAsRead, getAuditLogs, deleteAuditLogs, exportData, dismissBackupPrompt]);
